...
const orderFromApi = (order) => ({ id: `#${order.number}`, apiId: order.id, uberDelivery: order.uberDelivery, amendment: order.amendment, refund: order.refund, paidTotal: order.paidTotal, product: order.items.map((item) => item.name).join(", "), total: order.total, method: order.method, slot: order.slot, date: order.serviceDate ? new Intl.DateTimeFormat("fr-FR", { day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date(`${order.serviceDate}T12:00:00`)) : order.createdAt.slice(0, 10) === new Date().toISOString().slice(0, 10) ? "Aujourd’hui" : "Commande précédente", status: order.status, progress: progressForStatus[order.status] ?? 0 });
...
function AccessoryCard({ product, onPress }) {
  return <Pressable disabled={product.soldOut} onPress={() => onPress(product)} style={[styles.productCard, product.soldOut && styles.productCardSoldOut]}>
    {product.image ? product.kind === "drink" ? <View style={styles.drinkImageFrame}><Image source={product.image} style={styles.drinkProductImage} /></View> : <Image source={product.image} style={styles.productImage} accessibilityLabel={product.name} /> : <View style={styles.productEmojiImage}><Text style={styles.productEmoji}>{product.emoji}</Text></View>}
...
function CategoryHeader({ title, subtitle }) {
  return <View style={styles.categoryHeader}>
    <Text style={styles.categoryHeaderTitle} {...(Platform.OS === "web" ? { role: "heading", "aria-level": 2 } : { accessibilityRole: "header" })}>{title}</Text>
...
function SmsLoginScreen({ onBack, onAuthenticated }) {
  ...
  const checkCode = async () => {
    ...
    } catch (error) { setFeedback(error.message || 'Réessaie avec un nouveau code.'); Alert.alert("Code invalide", error.message || "Réessaie avec un nouveau code."); }
    finally { setLoading(false); }
  };
  if (reviewLogin) return <StoreReviewLogin api={API_BASE_URL} onBack={() => setReviewLogin(false)} onAuthenticated={onAuthenticated} />;
  if (registrationToken) return <CustomerIdentityScreen api={API_BASE_URL} registrationToken={registrationToken} onComplete={onAuthenticated} onExit={() => { setRegistrationToken(''); setCode(''); setStep('phone'); }} />;
...
function CheckoutDetailsScreen({ cart, customer, authToken, onChange, onCommentChange, onBack, onContinue, onEditIdentity }) {
  ...
  const calculateQuote = async () => {
    ...
    try {
      const response = await customerFetch(`${API_BASE_URL}/delivery-quote`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${authToken}` }, body: JSON.stringify(customer) });
      const payload = await response.json();
  ...
  const restaurantComment = <><Text style={styles.deliveryLabel}>COMMENTAIRE POUR LE RESTAURANT · FACULTATIF</Text><TextInput value={cart.comment || ""} onChangeText={onCommentChange} placeholder="Exemple : merci de bien cuire le steak" placeholderTextColor="#9B877B" style={styles.reservationNote} multiline maxLength={500} textAlignVertical="top" /></>;
...
function AppContent({ onReviewModeChange }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      loading = true;
      try {
        const response = await customerFetch(`${API_BASE_URL}/catalog`);
        const payload = await response.json();
  ...
  useEffect(() => { onReviewModeChange(isReviewToken(authToken)); }, [authToken, onReviewModeChange]);
  ...
  const loadCustomerReservations = async ({ token = authToken, silent = false } = {}) => {
    if (!token) return;
    try {
      const response = await customerFetch(`${API_BASE_URL}/customer/reservations`, { headers: { Authorization: `Bearer ${token}` } });
      const payload = await response.json();
  ...
  const loadCustomerRewards = async ({ token = authToken, silent = false } = {}) => {
    if (!token) return;
    try {
      const response = await customerFetch(`${API_BASE_URL}/customer/rewards`, { headers: { Authorization: `Bearer ${token}` } });
      const payload = await response.json();
  ...
  const authenticate = async ({ token, customer: savedCustomer, isNewCustomer = false }) => {
    ...
    setCustomer((current) => isReviewToken(token) ? { ...savedCustomer, distance: "", sponsorCode: "" } : ({
      ...savedCustomer,
  ...
  useEffect(() => {
    if (!authToken || isReviewToken(authToken)) return;
  ...
  useEffect(() => {
    ...
    customerFetch(`${API_BASE_URL}/customer/crm`, { headers: { Authorization: `Bearer ${authToken}` }, signal: controller.signal, cache: 'no-store' })
  ...
  const addToCart = (item) => {
    ...
    setCart((current) => {
      ...
      return { items, total: Math.round(items.reduce((sum, entry) => sum + entry.total, 0) * 100) / 100, comment: current?.comment || "", delivery: current?.delivery || { method: preferredMethod, fee: preferredMethod === "pickup" ? 0 : undefined, day: DEFAULT_DELIVERY_DAY.id, date: DEFAULT_DELIVERY_DAY.date, dayLabel: DEFAULT_DELIVERY_DAY.dayLabel, slot: null } };
    });
    setScreen("cart");
  };
  ...
  const continueWithCustomer = async (deliveryFee) => {
    ...
    if (API_BASE_URL) {
      try {
        ...
        const response = await customerFetch(endpoint, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${authToken}` }, body: JSON.stringify(customer) });
        const payload = await response.json().catch(() => ({}));
  ...
  const apiRequest = async (route, token, body) => {
    ...
    try {
      const response = await customerFetch(`${API_BASE_URL}${route}`, {
  ...
  const logoutCustomer = async () => {
    try {
      if (!isReviewToken(authToken)) await detachPushDevice(API_BASE_URL, authToken);
      await clearSession();
      sessionEpoch.current += 1;
  ...
  const deleteCustomerAccount = async () => {
    ...
    try {
      const response = await customerFetch(`${API_BASE_URL}/customer/account`, { method: "DELETE", headers: { Authorization: `Bearer ${authToken}` } });
      const payload = await response.json().catch(() => ({}));
  ...
  const claimLoyaltyReward = async (reward) => {
    ...
    try {
      const response = await customerFetch(`${API_BASE_URL}/customer/rewards/${encodeURIComponent(reward.id)}/claim`, { method: "POST", headers: { Authorization: `Bearer ${authToken}` } });
      const payload = await response.json().catch(() => ({}));
  ...
  const referFriend = async () => {
    if (isReviewToken(authToken)) { Alert.alert("Parrainage de test", "Le partage externe est désactivé dans cet espace isolé. Le parrainage client reste disponible hors mode de test."); return; }
