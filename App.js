...
function ReservationScreen({ customer, onBack }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/reservation-availability?date=${encodeURIComponent(day.date)}`)
      .then(async (response) => {
        ...
        if (slot && payload.slots?.[slot]?.unavailable) setSlot(null);
      })
  ...
  const submit = async () => {
    setSubmitting(true);
    try {
      const response = await fetch(`${API_BASE_URL}/reservations`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, phone, guests, serviceDate: day.date, slot, note }) });
      const payload = await response.json().catch(() => ({}));
...
export default function App() {
  ...
  if (screen === "reservation") return <ReservationScreen customer={customer} onBack={() => setScreen("menu")} />;
