...
const PROMOTIONS = [
  ...
  { id: "social", kind: "social", title: "Suivez-nous" },
...
function PromotionsCarousel({ onOpenProduct }) {
  ...
  useEffect(() => {
    // Keep the selected slide aligned after a device rotation or window resize.
    carouselRef.current?.scrollTo({ x: activeIndex * step, animated: false });
  }, [step]);
  ...
  return <View><ScrollView ref={carouselRef} horizontal snapToInterval={step} snapToAlignment="start" decelerationRate="fast" disableIntervalMomentum showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.promotionsTrack, { paddingRight: Math.max(0, contentWidth - cardWidth) }]} onScroll={(event) => setActiveIndex(Math.min(PROMOTIONS.length - 1, Math.max(0, Math.round(event.nativeEvent.contentOffset.x / step))))} scrollEventThrottle={16}>{PROMOTIONS.map((promotion) => {
    if (promotion.kind === "social") return <SocialLinks key={promotion.id} style={{ width: cardWidth }} />;
  ...
  })}</ScrollView><View style={styles.promotionDots}>{PROMOTIONS.map((promotion, index) => <Pressable key={promotion.id} accessibilityRole="button" accessibilityLabel={`Afficher ${promotion.title}, diapositive ${index + 1} sur ${PROMOTIONS.length}`} accessibilityState={{ selected: index === activeIndex }} onPress={() => carouselRef.current?.scrollTo({ x: index * step, animated: true })} style={styles.promotionDotButton}><View style={[styles.promotionDot, index === activeIndex && styles.promotionDotActive]} /></Pressable>)}</View></View>;
}
