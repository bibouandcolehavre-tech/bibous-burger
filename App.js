...
const POINT_EARNING_RULES = [
  ...
  { emoji: "✦", title: "Membre Bibou Plus", points: "Points ×2", detail: "Sur chaque commande pendant l’abonnement" },
];
...
const customerOrderPricing = (subtotal, standardDeliveryFee, customer, isDelivery = true) => {
  ...
  const discountRate = customerHasWelcomeReward(customer) ? WELCOME_DISCOUNT_RATE : bibouPlus ? BIBOU_PLUS_DISCOUNT_RATE : 0;
  const discount = Math.round(subtotal * discountRate * 100) / 100;
  const deliveryFee = bibouPlus && isDelivery ? 0 : standardDeliveryFee;
  return { bibouPlus, discountRate, discount, deliveryFee, total: Math.round((subtotal - discount + deliveryFee) * 100) / 100 };
};
...
const DRINK_PRODUCTS = [
  { id: "drink-coca", name: "Coca 33 cl", price: 1.8, kind: "simple", emoji: "🥤", description: "Canette 33 cl bien fraîche." },
  { id: "drink-coca-cherry", name: "Coca Cherry 33 cl", price: 1.8, kind: "simple", emoji: "🥤", description: "Canette 33 cl bien fraîche." },
  { id: "drink-fuse-menthe", name: "Fuse Tea thé vert menthe", price: 1.8, kind: "simple", emoji: "🧊", description: "Boisson rafraîchissante." },
  { id: "drink-lipton-framboise", name: "Lipton framboise 33 cl", price: 1.8, kind: "simple", emoji: "🧊", description: "Canette 33 cl bien fraîche." },
  { id: "drink-lipton-peche", name: "Lipton pêche 33 cl", price: 1.8, kind: "simple", emoji: "🧊", description: "Canette 33 cl bien fraîche." },
  { id: "drink-oasis-pomme", name: "Oasis pomme cassis framboise", price: 1.8, kind: "simple", emoji: "🥤", description: "Canette bien fraîche." },
  { id: "drink-oasis-tropical", name: "Oasis tropical", price: 1.8, kind: "simple", emoji: "🥤", description: "Canette bien fraîche." },
  { id: "drink-perrier", name: "Perrier", price: 1.8, kind: "simple", emoji: "💧", description: "Eau gazeuse bien fraîche." },
  { id: "drink-tropico", name: "Tropico", price: 1.8, kind: "simple", emoji: "🥤", description: "Canette bien fraîche." },
];
...
function AccessoryCard({ product, onPress }) {
  return <Pressable onPress={() => onPress(product)} style={styles.productCard}>
    {product.image ? <Image source={product.image} style={styles.productImage} /> : <View style={styles.productEmojiImage}><Text style={styles.productEmoji}>{product.emoji}</Text></View>}
    ...
    <ScrollView contentContainerStyle={styles.detailContent} showsVerticalScrollIndicator={false}>
      ...
      {!cart ? <View style={styles.emptyState}><Text style={styles.emptyIcon}>🛍</Text><Text style={styles.emptyTitle}>Ton panier est vide</Text><Text style={styles.emptyText}>Ajoute un burger qui te fait envie.</Text></View> : <>
        <View style={styles.cartItems}>{cart.items.map((item) => <View key={item.lineId} style={styles.cartItem}>
          {item.product.image ? <Image source={item.product.image} style={styles.cartImage} /> : <View style={styles.cartEmoji}><Text style={styles.cartEmojiText}>{item.product.emoji}</Text></View>}
          <View style={styles.productInfo}><Text style={styles.productName}>{item.product.name}</Text>{item.options.length > 0 && <Text numberOfLines={3} style={styles.productDescription}>{item.product.isMenu ? "Menu · " : ""}{item.options.join(", ")}</Text>}<Pressable onPress={() => onRemove(item.lineId)}><Text style={styles.cartRemove}>Retirer</Text></Pressable></View>
...
function DeliveryScreen({ cart, customer, onBack, onChange, onContinue, onOpenBibouPlus }) {
  ...
  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.detailContent} showsVerticalScrollIndicator={false}>
    <Header onBack={onBack} />
    <Text style={styles.title}>Quand veux-tu être livré?</Text>
    <Text style={styles.deliveryIntro}>Choisis le mode, le jour et le créneau qui t’arrangent.</Text>
    ...
    <Text style={styles.deliveryLabel}>CRÉNEAUX · {selectedDay.dayLabel.toUpperCase()}</Text>
    {availabilityStatus === "error" && delivery.method === "delivery" && <Text style={styles.availabilityError}>La disponibilité sera revérifiée avant le paiement.</Text>}
    <View style={styles.slots}>{selectedDay.slots.map((slot) => {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.loyaltyContent} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.pointsRuleFootnote}>Les points sont crédités uniquement après confirmation du paiement. Les boissons seules seront ajoutées au barème lorsqu’elles seront disponibles dans l’application.</Text>

        <Text style={styles.sectionTitle}>Ton bonus de la semaine</Text>
        <View style={styles.weeklyCard}>
          <View style={styles.weeklyHeader}>
            <View><Text style={styles.weeklyTitle}>{loyalty.orders} commande{loyalty.orders > 1 ? "s" : ""} cette semaine</Text><Text style={styles.weeklySubtext}>{loyalty.orders ? `Multiplicateur actuel × ${multiplier}${customerHasBibouPlus(customer) ? " · Bibou Plus ×2" : ""}` : "Ta première commande débloque tes points"}</Text></View>
...
export default function App() {
  ...
  const startBibouPlus = async () => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok || !payload.checkoutUrl) throw new Error(payload.error || "Impossible d’ouvrir le paiement Bibou Plus.");
      setPendingBibouPlus(payload.purchase);
      setScreen("bibou-plus-pending");
      await Linking.openURL(payload.checkoutUrl);
    } catch (error) {
      Alert.alert("Bibou Plus indisponible", error.message || "Réessaie dans un instant.");
  ...
  const checkBibouPlusPayment = async () => {
    ...
    try {
      ...
      if (payload.bibouPlus?.active) {
        setCustomer((current) => ({ ...current, ...payload.customer }));
        Alert.alert("Bibou Plus activé", "La livraison offerte, les 5 % de remise et les points doublés sont maintenant actifs.");
...
const styles = StyleSheet.create(applyAppPalette({
  ...
  bibouPlusShortcut: { marginTop: 12, maxWidth: 640, borderRadius: 18, padding: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#2C201B", borderWidth: 1, borderColor: "#5B4238" },
  bibouPlusShortcutCopy: { flex: 1, paddingRight: 10 },
  bibouPlusShortcutEyebrow: { color: "#FFB797", fontSize: 10, fontWeight: "900", letterSpacing: 1 },
  bibouPlusShortcutTitle: { color: "#FFFFFF", fontSize: 15, fontWeight: "900", marginTop: 5 },
  bibouPlusShortcutPrice: { color: "#F2C8B5", fontSize: 12, fontWeight: "700", marginTop: 4 },
  bibouPlusShortcutArrow: { color: "#E95122", fontSize: 31, lineHeight: 32 },
  ...
  pointsRuleFootnote: { color: "#826E63", fontSize: 11, lineHeight: 16, marginTop: 10 },
