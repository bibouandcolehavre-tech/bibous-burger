...
function SmsLoginScreen({ onBack, onAuthenticated }) {
  ...
  const checkCode = async () => {
    setLoading(true);
    try {
      const response = await customerFetch(`${API_BASE_URL}/auth/sms/check`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone, code }) });
      const payload = await response.json();
...
function CheckoutDetailsScreen({ cart, customer, authToken, onChange, onCommentChange, onBack, onContinue }) {
...
function AppContent({ onReviewModeChange }) {
  ...
  if (screen === "details") return <CheckoutDetailsScreen cart={cart} customer={customer} authToken={authToken} onChange={setCustomer} onCommentChange={(comment) => setCart((current) => ({ ...current, comment }))} onBack={() => setScreen("delivery")} onContinue={continueWithCustomer} />;
