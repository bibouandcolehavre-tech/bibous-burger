...
- Lors du déploiement SEO seul, la propriété existante `bibousburgers.com` et l'ancienne boutique SumUp n'ont pas été modifiées. La redirection du domaine a ensuite été demandée et réalisée séparément (voir ci-dessous). Aucun moyen de paiement ou abonnement changé.
...
### Ancienne adresse et domaine — diagnostic du 24 septembre

Le propriétaire a corrigé sa demande : les QR codes imprimés pointent vers la fiche Google du restaurant et ne sont pas à modifier. Il souhaite uniquement rediriger l'ancienne adresse du site vers l'application web. Contrôle réel dans le navigateur : `https://bibou-s-burgers.sumupstore.com/` redirige déjà vers **`https://bibousburgers.com/`**, qui affiche **« Boutique temporairement fermée »**.

Les serveurs DNS publics sont `nsa1.squarespacedns.com` à `nsa4.squarespacedns.com`. L'ancien A `108.128.115.77` a été remplacé par le service de redirection Squarespace. Les MX sont ceux de Google Workspace : **messagerie conservée**, contrôle public des cinq valeurs identiques après intervention.

Le propriétaire s'est connecté à Squarespace et a explicitement confirmé la redirection permanente du domaine et de ses anciennes pages vers l'accueil de l'application. Après retrait du seul A web en conflit, Squarespace a confirmé la création de la règle 301 sans conservation des chemins. Deux essais navigateur ont réussi : ancienne racine SumUp et ancienne fiche Menu À travers les montagnes arrivent sur l'application avec sa carte visible. Squarespace annonce 24–48 h pour la propagation complète ; un autre client réseau présentait encore un délai HTTPS. Les valeurs de restauration et les résultats précis figurent dans `DOMAIN_REDIRECTION.md`. Ne pas supprimer ni déconnecter la boutique SumUp : l'ancienne adresse SumUp dépend encore de cette liaison. Le service de paiement SumUp reste intact.
