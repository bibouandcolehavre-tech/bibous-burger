...
- Aucun compte de service privé/clé FCM n'a été créé ou transmis à Expo. Un accès dédié aux notifications est préparé ; la création et le transfert sécurisé à EAS font l'objet d'une demande de confirmation distincte. Le seul compte actuellement constaté est le compte Firebase Admin SDK automatiquement créé par Google ; ne pas partager ses droits administrateur globaux avec Expo. Le rôle officiel `roles/firebasecloudmessaging.admin` a été inspecté en lecture seule. Ne jamais demander de coller une clé privée dans le chat, Git ou le binaire.
...
1. **Android** : la configuration publique `google-services.json` du projet `bibou-s-burgers` et du package `com.krokly.bibousburgers` est intégrée. Reste à associer à EAS les identifiants FCM v1 autorisés. La clé de compte de service privée doit rester hors de Git et hors du binaire. Sa création/son accès nécessitent l’autorisation appropriée.
