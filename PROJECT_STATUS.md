...
- Reprise des versions de test explicitement autorisée le 19 septembre après la livraison du système push. Le propriétaire utilise un **iPhone**, désormais prioritaire. La pause mentionnée dans les entrées historiques ci-dessous est levée pour les tests seulement ; aucune publication Apple/Google ni nouvelle dépense autorisée. Connexion Apple réalisée et contrat gratuit « Apple Developer Agreement » accepté avec autorisation explicite ; communications facultatives désactivées. **L’adhésion Apple Developer Program payante n’est pas active**, le compte propose de la rejoindre. Aucun binaire iOS signé créé (historique Expo vide), aucun achat ni nouvelle clé. Les fichiers privés de signature sont désormais exclus de Git, de l’archive source Expo et de Docker. Ne pas redemander l’autorisation de reprendre les builds ; voir `RELEASE_READINESS.md`.

- Notifications clients préparées le 19 septembre, **envois réels désactivés** : « Mon compte → Mes notifications », choix suivi/promotions séparés et non précochés ; espace restaurant « Notifications clients », aperçu, audience tous/Bibou +, confirmation et historique. Statuts de commandes payées et tables raccordés à une file persistante avec priorité au suivi, dédoublonnage, péremption, gestion des tickets/reçus et nettoyage des appareils. Aucun SMS, paiement ou push réel pendant les tests.
- Vérification push : 161 tests au total (dont consentement, suppression/déconnexion, droits API, changement de propriétaire, faux fournisseur, navigation native simulée, aperçu sans envoi et réponse tardive après déconnexion). Exports web/iOS/Android réussis, Expo Doctor 21/21. Interfaces vérifiées à 390 px et sur bureau avec données fictives. **Ces exports ne sont pas des binaires signés et ne valident pas une réception réelle.**
- Blocage push concret : la page Credentials du projet Expo a été consultée, aucun identifiant Apple/Android associé ; `google-services.json` absent. La pause initiale sur les versions installables est désormais levée pour les tests (voir l’entrée du 19 septembre ci-dessus). Guide complet dans `PUSH_NOTIFICATIONS.md` : accès FCM/APNs à valider, builds puis test sur téléphone avant activation serveur. Les alertes web restaurant restent des sons dans une page ouverte, pas du Web Push. Aucune adhésion payante supplémentaire souscrite. L’inscription Apple attend les informations personnelles du propriétaire ; détails dans `RELEASE_READINESS.md`.
- Les retouches des photos de burgers sont reportées à demain à la demande du propriétaire ; aucune photo de l’application modifiée pendant le chantier push.
...
- Accueil : le carrousel est désormais « Nos actualités », sans carte de burger. Ordre confirmé par le propriétaire : concours en préparation, vidéo Epicu du 2 mai 2026 (Reel DX1QN8DIdet), vidéo Paris-Normandie du 13 juin 2025 (Facebook /presse.havraise/videos/bibous-burger/1226219005580139/, date et compte vérifié contrôlés), puis Suivez-nous. L’article Étoiles gourmandes et sa photo sont retirés du carrousel. Les burgers restent dans la carte des produits.
...
6. Reprise autorisée, iPhone prioritaire : après connexion Apple et vérification de l’adhésion/signature et du quota Expo, générer la version de test puis effectuer un test sur appareil réel. Préparer ensuite Android. Le compte Expo est connecté et le projet associé ; les exports JavaScript ne remplacent pas ces builds signés. EAS Update n’est pas encore configuré : ne pas promettre des mises à jour natives à distance déjà actives.
