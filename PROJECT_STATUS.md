...
- Blocage push concret : la page Credentials du projet Expo a été consultée, aucun identifiant Apple/Android associé ; `google-services.json` absent. La pause demandée sur les versions installables est respectée. Guide complet dans `PUSH_NOTIFICATIONS.md` : connexions FCM/APNs autorisées, reprise des builds puis test sur téléphone avant activation serveur. Les alertes web restaurant restent des sons dans une page ouverte, pas du Web Push. Aucun service payant ni compte supplémentaire souscrit.
